...
gulp.task("compress", () => {
  return Promise.all(
    nw.options.platforms.map(platform => {
      ...
      if (platform.indexOf("win") !== -1 || platform.indexOf("darwin") !== -1) {
      ...
      return new Promise((resolve, reject) => {
        ...
        if (currentPlatform().indexOf("win") !== -1) {
          return gulp
            ...
            .pipe(
              glp.tar(
                pkJson.name + "-" + pkJson.version + "_" + platform + ".tar"
              )
            )
            .pipe(glp.gzip())
            .pipe(gulp.dest(releasesDir))
          ...
          const platformCwd =
            platform.indexOf("linux") !== -1 ? "." : pkJson.name + ".app";
          ...
          const commands = [
            ...
            "tar --exclude-vcs -c " +
              platformCwd +
              ' | $(command -v pxz || command -v xz) -T8 -7 > "' +
              path.join(
                process.cwd(),
                releasesDir,
                pkJson.name + "-" + pkJson.version + "_" + platform + ".tar.xz"
              ) +
              '"',
            'echo "' +
              platform +
