<table>
    ...
    <tr class="item-row" data-key="<%=k %>">
        ...
        <% if (torrent.quality) { %><td class="info item-play"><%=torrent.quality %></td><% } %>
        <% if (torrent.filesize) { %><td class="info item-play"><%=torrent.filesize %></td><% } %>
        <% if (Settings.activateSeedbox) { %><td class="action tooltipped" title="<%= i18n.__('Download') %>" data-toggle="tooltip" data-container="body" data-placement="top"><i class="fa fa-download item-download"></i></td><% } %>
    </tr>
    <% }); %>
</table>
