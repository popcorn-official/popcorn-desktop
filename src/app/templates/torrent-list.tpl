<table>
    ...
    <tr class="item-row" data-key="<%=k %>">
        <td class="provider tooltipped" title="<%=torrent.source ? torrent.source.split('//').pop().split('/')[0] : torrent.provider.toLowerCase() %>" <% if (!torrent.source) { %>style="cursor:default" <% } %>data-toggle="tooltip" data-container="body" data-placement="left"><img data-href="<%=torrent.source %>" src="<%=torrent.icon %>" <% if (!torrent.source) { %>style="cursor:default" <% } %>onerror="this.onerror=null; this.style.display='none'; this.parentElement.style.top='0'; this.parentElement.classList.add('fas', 'fa-link')" onload="this.onerror=null; this.onload=null;"/></td>
